...
### 로컬 테스트
docker가 실행된 상태에서 `localstack start` 명령으로 LocalStack을 구동합니다. 이후 터미널에 `Ready`가 나타나면 다른 터미널 창을 열고, 프로젝트 디렉토리에서 아래 명령을 수행합니다.
```bash
